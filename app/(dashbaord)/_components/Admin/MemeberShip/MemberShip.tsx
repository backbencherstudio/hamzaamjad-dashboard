...
import { useMembershipContext } from '@/hooks/useMembership'
...
function MembershipPageContent() {
    ...
    const tabs = [
        ...
        },
        {
            key: 'subscribed',
            label: 'Subscribed',
            count: memberships.filter(member => getSubscriptionStatusText(member) === 'Subscribed').length
        },
        {
            key: 'expired',
            label: 'Expired',
            count: memberships.filter(member => getSubscriptionStatusText(member) === 'Expired').length
        },
        {
            key: 'non-subscribed',
            label: 'Non Subscribed',
            count: memberships.filter(member => getSubscriptionStatusText(member) === 'Non Subscribed').length
        }
    ];
    ...
    const filteredData = useMemo(() => {
        ...
        if (activeTab !== 'all') {
            if (activeTab === 'subscribed') {
                filtered = filtered.filter(member => getSubscriptionStatusText(member) === 'Subscribed');
            } else if (activeTab === 'expired') {
                filtered = filtered.filter(member => getSubscriptionStatusText(member) === 'Expired');
            } else if (activeTab === 'non-subscribed') {
                filtered = filtered.filter(member => getSubscriptionStatusText(member) === 'Non Subscribed');
            }
        }
        return filtered;
    ...
    const handleTabChange = (tabKey: string) => {
        ...
        setPage(1);
        let newStatus = '';
        if (tabKey === 'subscribed') newStatus = 'ACTIVE';
        else if (tabKey === 'expired') newStatus = 'DEACTIVE';
        else if (tabKey === 'non-subscribed') newStatus = 'EMPTY';
        
        setStatus(newStatus);
        fetchMemberships(1, limit, debouncedSearchTerm || undefined, newStatus);
    };
